...
def build_slides(photos, svgs, tmp: Path) -> list[Slide]:
    ...
    # 5 · Новое: см-оценка от высоты съёмки (раздел 6 HTML, кадры est(H))
    s = Slide()
    s.title("6", "Новое: см-оценка глубины на рабочих кадрах",
            "Эталона в кадре нет, но известна высота съёмки (1,70 м) — масштаб "
            "полотна строится по геометрии кадра. Всегда ОЦЕНКА: полоса ошибки, "
            "confidence=low, метка «не для актирования».")
    # три кадра est(H) — последние три data-фото HTML, идут сразу за «Яма 012»
    idx012 = next(i for i, (a, _) in enumerate(photos) if a.startswith("Яма 012"))
    est_cards = [
        ("яма 047 · d ≈ 2,1 см · полоса 1,2–3,5",
         "Двор: выкрошенная яма ~78 см поперёк; порог ГОСТ 5 см не превышен."),
        ("яма 015 · d ≈ 2,3 см · полоса 1,2–4,3",
         "Перекрёсток: скол ~93 см; широкая полоса — честное следствие "
         "оценочного масштаба от высоты камеры."),
        ("яма 022 · d ≈ 0,9 см · полоса 0,5–1,7",
         "Проезд у гаражей: глубина под сантиметр, ГОСТ-порога не достигает."),
    ]
    x = 1.2
    for (head, body), (_, data) in zip(est_cards, photos[idx012 + 1:idx012 + 4]):
        place_photo(s, data, x, 3.9, w_cm=10.0)
        s.text(cm(x), cm(10.6), cm(10.0), cm(3.4),
               [(head, 12.5, True, INK, "l", 4), (body, 11, False, MUT, "l", 0)])
        x += 10.7
    s.text(cm(1.2), cm(14.4), cm(31.4), cm(3.0),
           [("Контрольный прогон по 28 парам: оценку получили 22 из 31 дефекта; "
             "где геометрия кадра проверок не прошла — честный отказ, чисел нет. "
             "Сертифицируемых сантиметров этот режим не даёт — метка est(H) "
             "стоит прямо на кадре.", 13, False, INK, "l", 0)],
           fill=BOX_FILL, line=BOX_LINE)
    S.append(s)

    # 6 · Почему фото не даёт сантиметров (схема 0)
    ...
    # 7 · Путь к сантиметрам — обзор трёх ступеней
    s = Slide()
    ...
    s.text(cm(1.2), cm(13.9), cm(31.4), cm(2.6),
           [("Общее правило: каждое число выходит с полосой погрешности; если "
            ...
            ("Уже сегодня, без новых выездов: категория «мелкая / средняя / "
             "глубокая» для каждой ямы; при люке ГОСТ 3634 в кадре или известной "
             "высоте съёмки — оценка d ≈ rel·W⊥ с полосой и пометкой «оценка, "
             "не для акта».",
             12, False, MUT, "l", 0)])
    S.append(s)

    # 8–10 · этапы со схемами 1..3
    S.append(schema_slide(
    ...
    rows = [
        ("Сертифицируемые см — только с эталоном",
         "Размер в см для акта — лишь при объекте известного размера в кадре "
         "(люк ГОСТ 3634, разметка, линейка). Высота съёмки даёт только "
         "ОЦЕНКУ — с полосой и меткой «не для актирования»."),
        ("Измеренную глубину фото не даёт",
         "По обычному снимку — категория «мелкая / средняя / глубокая» и, при "
         "известной высоте съёмки, явная оценка с полосой. Измеренные "
         "сантиметры — из кадра «рейка + рулетка» или серии с доской."),
