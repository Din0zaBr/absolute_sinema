...
Сцены: соседние ямы сборщик снимает одним общим кадром (колонка scene
журнала, пишет ingest_local_pairs). С 2026-07-16 ингест дедуплицирует пары —
в local_pairs одна пара на сцену, отчёты «по парам» и «по сценам» совпадают;
на СТАРЫХ прогонах (до дедупа) пары одной сцены — одни и те же фото, признак
«найдено» у них общий, поэтому recall считается и по парам, и по сценам.
